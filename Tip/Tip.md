...
# 언제 DFS? BFS? 백트래킹? 다이나믹?
1️⃣ DFS / BFS / 백트래킹이 필요한 경우<br>
✅ 핵심 조건
- 가능한 모든 조합·경로·순서를 탐색해야 함
- 한 번의 선택이 다음 단계에 영향을 줌 (순차적 결정 문제)
- 상태의 “중복 재사용”이 거의 없음
→ 즉, 같은 상황을 여러 경로로 도달하지 않음<br>

🧠 판단 포인트
- “모든 경우를 봐야 하는데 N이 작다” → DFS/백트래킹
- “순서나 조합에 따라 결과가 달라진다” → DFS/백트래킹
- “그래프 구조, 인접 노드 이동” → DFS/BFS

2️⃣ DP(동적 계획법)이 필요한 경우<br>
✅ 핵심 조건

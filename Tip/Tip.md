...
비트마스크(bit mask)는 정수의 각 비트를 ‘ON/OFF 스위치’처럼 사용해서 여러 상태를 한 변수로 관리하는 기법이야.
파이썬에서도 정수 연산으로 간단하게 쓸 수 있고, 색칠 문제 같은 겹침 판단이나 집합 연산을 할 때 정말 유용해.

### 1. 기본개념

| 개념                 | 설명                      |
| ------------------ | ----------------------- |
| **비트(bit)**        | 0 또는 1로 표현되는 데이터의 최소 단위 |
| **마스크(mask)**      | 특정 비트만 선택하거나 제어하기 위한 값  |
| **비트마스크(bitmask)** | 여러 상태를 각 비트에 저장한 정수     |
...
| 연산자  | 이름      | 의미                  | 예시                            |  
| ---- | ------- | ------------------- | ----------------------------- | 
| `&`  | AND     | 두 비트가 모두 1일 때만 1    | `6 & 3 = 2` (110 & 011 = 010) |   
| `    | `       | OR                  | 둘 중 하나라도 1이면 1                |  
| `^`  | XOR     | 두 비트가 다르면 1         | `3 ^ 1 = 2` (011 ^ 001 = 010) |   
| `~`  | NOT     | 비트를 반전(1→0, 0→1)    | `~1 = -2` (보수 연산)             |    
| `<<` | 왼쪽 시프트  | 비트를 왼쪽으로 이동(2배씩 증가) | `1 << 3 = 8` (0001 → 1000)    |   
| `>>` | 오른쪽 시프트 | 비트를 오른쪽으로 이동(2로 나눔) | `8 >> 3 = 1`                  |    
